...
export async function generateNicheAnalysis(answers) {
...
export async function generateViralAnalysis(input, youtubeData = null) {
  const model = genAI.getGenerativeModel({
    model: 'gemini-2.0-flash',
    generationConfig: {
      responseMimeType: 'application/json'
  ...
  if (youtubeData && youtubeData.videos && youtubeData.videos.length > 0) {
    youtubeContext = `
    DADOS REAIS DO YOUTUBE (use essas informações na sua análise):
    ${JSON.stringify(youtubeData.videos.slice(0, 10).map(v => ({
      title: v.title,
